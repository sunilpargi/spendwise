...
const HomePage = () => {
    ...
    // Add debug logs for filtered transactions
    useEffect(() => {
        console.log(`Filtered Transactions: ${JSON.stringify(filteredTransactions)}`);
        console.log(`Total Income: ${totalIncome}, Total Expense: ${totalExpense}, Total Available Balance: ${totalAvailableBalance}`);
        console.log(`Selected Envelope: ${JSON.stringify(selectedEnvelope)}`);
    }, [filteredTransactions, totalIncome, totalExpense, totalAvailableBalance, selectedEnvelope]);
    return (
